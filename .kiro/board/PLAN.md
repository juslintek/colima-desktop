...
|----|------|-------------|-----------|-------|--------|
| M0.1 | Board substrate + seed README/LICENSE | orchestrator/architect | — | `.kiro/board/`, `README.md`, `LICENSE` | WIP |
| M0.1b | Port 10 skills + scaffold 10 agents | architect | — | `~/.kiro/skills/`, `~/.kiro/agents/` | TODO |
| M0.2 | Extract `ColimaDesktopKit`; fix Xcode-26 hang | swiftui-dev | — | `Sources/`, `project.yml` | TODO |
| M0.3 | Coverage wiring + `verify.sh` scoreboard | devops | M0.2 | `scripts/verify.sh`, `Makefile` | TODO |
| M0.4 | Finalize proto; daemon-backed mac provider; FREEZE CONTRACT v1 | go-daemon-dev + architect | M0.2 | `proto/`, `daemon/`, `Sources/Services/` | TODO |
| M1.5 | Daemon backend completeness + parity-matrix + gRPC integ tests | go-daemon-dev | M0.4 | `daemon/`, `docs/parity-matrix.md` | TODO |
| M2.6 | Windows WinUI 3 GUI | windows-native-dev | M0.4 | `windows/` | TODO |
| M2.7 | Linux GTK4 GUI | linux-native-dev | M0.4 | `linux/` | TODO |
| M2.8 | TUI Bubble Tea | tui-dev | M0.4 | `tui/` | TODO |
