...
- **outcome:** DONE ✅ (commit 8fab174). Root cause confirmed from GTK4 4.22.4 binary strings: valid `GTK_A11Y` values are `accesskit|atspi|test|none|help`; `GTK_A11Y=1` produces "Unrecognized accessibility backend '1'" and prevents AT-SPI registration. All three files updated to `GTK_A11Y=atspi`. YAML/Python/shell all validate clean. Nonzero hard gate preserved in workflow. ground-truth.json updated with diagnostic record. Committed; not pushed.

### 2026-07-18T14:09Z · windows-ui-explorer · M3.9 diagnostic pass 4
- **intent:** Fix WindowsAppSDK "SelfContained platform AnyCPU" error (run 29641947428). Root cause: MSBuild `$(Platform)` defaults to `AnyCPU` because `<Platform>` was never set in the csproj PropertyGroup, and the CI workflow did not pass `-p:Platform=x64`. The existing `<RuntimeIdentifier>`, `<PlatformTarget>`, and `<Platforms>` properties do not override MSBuild's internal `$(Platform)` evaluation target that gates SelfContained resolution.
- **plan:** (1) `windows/ColimaDesktop.Windows.csproj` — add `<Platform>x64</Platform>` to the main PropertyGroup; (2) `.github/workflows/explore-windows.yml` — add `-p:Platform=x64 -p:PlatformTarget=x64` to both the `dotnet restore` and `dotnet publish` steps for the app; (3) validate YAML and XML; commit; no push.
- **files-to-touch:** `windows/ColimaDesktop.Windows.csproj`, `.github/workflows/explore-windows.yml`.

### 2026-07-18T14:09Z · linux-ui-explorer · M3.9 diagnostic pass 4
- **intent:** Fix CI exploration stuck in onboarding window — all 11 captures were the same onboarding UI because `colima` is absent in the CI runner, causing `DependencyManager::is_colima_installed()` (which uses `which::which("colima")`) to return false and `build_ui()` to call `build_onboarding_window()` instead of `build_main_window()`. Root cause confirmed from `linux/src/main.rs` L49-53 and `dependency_manager.rs` L62. No onboarding sidebar exists — all 11 surfaces were duplicate onboarding captures.
- **plan:** (1) Create `scripts/linux/colima_shim.sh` — a CI-only `colima` shim that handles `--version`/`version` (print version string, exit 0), `start`/`stop`/`status`/`list`/`delete` (print JSON stub, exit 0), and exits 1 for unrecognized subcommands. (2) Update `scripts/linux/run_explore.sh` — create a temp bin dir, copy the shim as `colima`, prepend it to PATH before launching the explorer; clean up after. (3) Update `.github/workflows/explore-linux.yml` — same: create `/tmp/ci-bin/colima` shim and prepend to PATH in the explore step env. (4) Update `scripts/linux/explore_atspi.py` — (a) detect onboarding window and fail fast (checking accessible label/widget_name `window_onboarding` or title containing "Setup"), (b) improve sidebar locator: search by widget_name `sidebar_list` OR accessible property label `Navigation` OR `LIST_BOX` role, (c) require at least 2 distinct non-empty surfaces (not just 1), (d) emit clear structured error if onboarding window is still showing. (5) Update `exploration/linux/ground-truth.json` with new diagnostic record.
- **files-to-touch:** `scripts/linux/colima_shim.sh` (new), `scripts/linux/run_explore.sh`, `scripts/linux/explore_atspi.py`, `.github/workflows/explore-linux.yml`, `exploration/linux/ground-truth.json`.
- **outcome:** (pending)
