...
All 12 surfaces were captured using real binary execution in a pseudo-terminal (PTY) plus
...
- **PTY screenshots**: The real `colima-tui` binary was spawned in a pseudo-terminal
  ...
  so the TUI unblocked and rendered. Navigation keys (1-9, 0, →, and Monitoring key) were
  sent to traverse all 12 tabs. Raw ANSI frames were captured and stored in
  `screenshots/*.ansi`.
...
- **Model-direct content**: `go run driver_explore.go` inside `tui/` drives the
  ...
  (containers list, images list, config values, monitoring stats, etc.) with distinct
  fingerprints per tab. This is the source of `normalized_frame` and
  `content_fingerprint` in the JSON.
...
|11 | Monitoring     | `→→`| VM Monitoring: CPU/Memory/Disk stats, process kill |

## Files

```
ground-truth.json             — Main ground-truth record (12 surfaces, all validated)
...
screenshots/11_monitoring.*   — Monitoring view
```
...
- `total_surfaces`: 12 ✓
- `all_nonempty`: true ✓
- `all_distinct`: true ✓ (all 12 content fingerprints are unique)
- `validation_pass`: true ✓
- `errors`: [] ✓ (zero errors)

## Evidence Limitations

- **STUB DATA**: fakeDS DataSource injects mock rows (containers, images, profiles,
  machines, monitoring stats). These are NOT live daemon responses. Purpose: confirm
  surfaces render populated tables. Data accuracy is not validated.
- **No accessibility tree**: PTY character grids are the only introspection method.
  Semantic element roles and interactive element boundaries are not capturable.
- **No live daemon**: The `colima-daemon` gRPC socket was absent during capture. All
  backend-dependent data is stub.
...
- Monitoring surface was added after DISC-03 resolution; it renders VM resource
  usage (CPU/Memory/Disk), process list, and PID kill — matching CONTRACT Part A
  VMStats/ProcessList/KillProcess RPCs.
