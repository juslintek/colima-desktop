...
Order of operations:
1. SSH into tart-vm, run `brew install colima docker` (CLI only — NEVER Docker Desktop), then `colima start --vm-type vz --mount-type virtiofs` in background. Wait for socket at ~/.colima/default/docker.sock.
...
5. Run tests in VM via `ssh tart-vm`, fix failures, repeat until all pass.
...
- Test: `ssh tart-vm "cd '/Volumes/My Shared Files/project' && xcodebuild test -scheme ColimaDesktop -destination 'platform=macOS' -derivedDataPath /tmp/DD -only-testing:ColimaDesktopUnitTests"`
