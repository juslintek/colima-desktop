...
struct ImageBrowserSheet: View {
    ...
    @State private var hubResults: [(name: String, description: String, stars: Int, official: Bool)] = []
    ...
    private func searchHub() {
        let term = searchText.isEmpty ? "popular" : searchText
        Task {
            if let raw = try? await appState.services.searchImages(term: term) {
                let rows = raw.map { item in
                    (name: item["name"] as? String ?? "",
                     description: item["description"] as? String ?? "",
                     stars: item["star_count"] as? Int ?? 0,
                     official: item["is_official"] as? Bool ?? false)
                }
                await MainActor.run { hubResults = rows }
            }
        }
    }
