...
pub fn build() -> GtkBox {
    ...
    dep_box.set_margin_start(24); dep_box.set_margin_end(24);
    dep_box.set_margin_top(16);

    let statuses = DependencyManager::check_all();

    let dep_labels: Vec<Label> = statuses.iter().map(|s| {
        ...
        if s.installed { status_lbl.add_css_class("success"); }
        else { status_lbl.add_css_class("error"); }
    ...
    action_row.set_margin_top(16); action_row.set_margin_bottom(16);

    let btn_install_colima = make_action_button("⬇ Install Colima", "onboarding_btn_install_colima");
    ...
    // Install Colima
    {
        ...
        btn_install_colima.connect_clicked(move |_| {
            ...
            // Run in a separate thread so GTK loop doesn't block
            let lb2 = lb.clone();
            std::thread::spawn(move || {
                ...
                glib::idle_add_once(move || set_text(&lb2, &msg));
            });
        });
    }
    ...
    // Install All
    {
        ...
        btn_install_all.connect_clicked(move |_| {
            ...
            std::thread::spawn(move || {
                ...
                glib::idle_add_once(move || set_text(&lb2, &log));
            });
        });
    }
    ...
    // Update All
    {
        ...
        btn_update_all.connect_clicked(move |_| {
            ...
            std::thread::spawn(move || {
                ...
                if log.is_empty() { log = "No installed dependencies to update.".to_owned(); }
                glib::idle_add_once(move || set_text(&lb2, &log));
            });
        });
    }
