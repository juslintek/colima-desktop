...
pub fn build(handle: AppHandle) -> GtkBox {
    ...
    // Pull — streaming; collect all log lines in the Tokio task, send as a single String
    {
        let h = handle.clone();
        let lb = log_buf.clone();
        let sp = spinner.clone();
    ...
    // Search
    {
        ...
        btn_search.connect_clicked(move |_| {
            ...
            if let Some(ref mut client) = state.daemon {
                ...
                h.rt.spawn(async move {
                    let result = c
                        .search_images(SearchRequest { term, profile })
                        .await
                        .map(|r| {
                            let j = r.into_inner();
                            if j.error.is_empty() {
