...
pub fn build(handle: AppHandle) -> GtkBox {
    ...
    exec_row.set_margin_start(12); exec_row.set_margin_end(12);
    exec_row.set_margin_top(8); exec_row.set_margin_bottom(8);

    let entry_cmd = Entry::builder().placeholder_text("kubectl command (e.g. get pods)").hexpand(true).build();
    ...
    exec_row.append(&entry_cmd); exec_row.append(&btn_exec);
            ...
            $btn.connect_clicked(move |_| {
                ...
                if let Some(ref mut client) = state.daemon {
                    ...
                    h.rt.spawn(async move {
                        let res = c.$rpc(ProfileRequest { profile }).await;
    ...
    // Exec
    {
        ...
        btn_exec.connect_clicked(move |_| {
            ...
            if command.is_empty() { set_text(&lb, "Enter a kubectl command first"); return; }
            sp.set_spinning(true);
            let profile = h.profile();
            let mut state = h.state.lock().unwrap();
            if let Some(ref mut client) = state.daemon {
                let mut c = client.colima.clone(); let lb2 = lb.clone(); let sp2 = sp.clone();
                h.rt.spawn(async move {
                    let res = c.kubernetes_exec(KubeExecRequest { profile, command }).await;
                    glib::idle_add_once(move || {
                        sp2.set_spinning(false);
                        match res {
                            Ok(r) => {
                                let resp = r.into_inner();
                                if resp.error.is_empty() { set_text(&lb2, &resp.output); }
                                else { set_text(&lb2, &format!("{}\n{}", resp.output, resp.error)); }
                            }
